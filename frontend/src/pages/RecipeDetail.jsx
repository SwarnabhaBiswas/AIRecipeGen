...
const RecipeDetail = () => {
  ...
  const handleDelete = async () => {
    console.log("DELETE CLICKED"); // 👈 ADD THIS

    if (!confirm("Are you sure you want to delete this recipe?")) return;

    console.log("CONFIRMED"); // 👈 ADD THIS
          ...
          <div className="lg:col-span-2 space-y-6">
                ...
                {recipe.instructions &&
                  recipe.instructions.map((step, index) => (
                    <li key={index} className="flex gap-4">
                      <span className="shrink-0 w-8 h-8 bg-primary-500 text-white rounded-full flex items-center justify-center text-sm font-semibold">
                        {index + 1}
                      </span>
            ...
            {recipe.user_notes && (
              <div className="bg-primary-50 rounded-xl border border-primary-200 p-6">
                ...
                <p className="text-primary-800">{recipe.user_notes}</p>
              </div>
            )}
          </div>
        </div>
